# Unit 09 Student Packet: Flavor Systems

## What We Are Learning
We are learning how flavor is built. You will practice safe tasting, salt, acid, fat, aromatics, herbs, spices, balance, small adjustments, and respectful flavor comparison.

## Why We Are Learning It
Recipes tell you what to do, but cooks also need to notice flavor. Good flavor work means tasting safely, adjusting carefully, and respecting where flavor ideas come from.

## What Success Looks Like
- I can describe how salt, acid, fat, aromatics, herbs, and spices affect food.
- I can taste safely.
- I can adjust in small steps.
- I can use specific flavor words.
- I can compare flavor builds respectfully.
- I can keep tasting stations clean and sanitary.

## Daily Kitchen Entry Routine
1. Enter calmly.
2. Put belongings away.
3. Check the agenda.
4. Check uniform/readiness.
5. Join the huddle.
6. Wash hands when directed.
7. Begin work only when released.

## Key Expectations
- Use clean tasting spoons.
- Do not double-dip.
- Add small amounts.
- Use specific words.
- Respect cultural origins without pretending our samples are authentic.
- Clean affected surfaces carefully.

## Kitchen Communication
- "Clean spoon"
- "Dirty spoon"
- "Too salty"
- "Needs acid"
- "Heard"
- "I need help"

## Safety Expectations
- Follow tasting rules every time.
- Keep tasting tools organized.
- Wash hands and sanitize surfaces.
- Use hot-pan safety during aromatic work.
- Ask before tasting if unsure.

## Flavor Practice Expectations
- Taste the base first.
- Add one small change.
- Taste again with a clean spoon.
- Describe what changed.
- Adjust only with teacher-approved ingredients.
- Clean and reset before leaving.

## Cleanup and Reset Checklist
- [ ] Tasting spoons/cups handled correctly
- [ ] Counters cleaned and sanitized
- [ ] affected surfaces cleaned
- [ ] Ingredients closed and stored
- [ ] Tools washed
- [ ] Towels handled correctly
- [ ] Trash checked
- [ ] Floor checked
- [ ] Station ready for the next group

## Bistro Connection
Bistro food should be checked before it goes out. Safe tasting, small adjustments, and clear communication help protect guests and improve consistency.

## Student Reflection Questions
1. What flavor word did you use today?
2. What small adjustment changed the food?
3. What tasting rule did you follow?
4. How did your group keep tasting tools controlled?
5. How can you describe flavor respectfully?
