...
Life of an Order is not only a sequence to memorize. Every Bistro team learns to recognize the order and guest experience unfolding in real time and asks:
...
## Before the First Order — Graceful Urgency

Guests may already be present when students arrive. Students move from the school day into Bistro with **graceful urgency**: purposeful movement, calm communication, and immediate action without running, panic, or chaos.

Current Work Ready arrival language is:

**ID → Clock In → Lock In → Park It → Stow Belongings → Notebook → Pen → A Plan → Report to Station → Open**
...
The Service host is the face of the Bistro. Greet every student first with **“Good morning.”** Do not decide whether the student belongs before greeting.
...
Guests seat themselves at any available spot. Service does not assign tables or escort guests to a specific seat. Service keeps the room ready for self-seating: clean tables, straight chairs, clear floors, available service items, and no confusing or unusable spaces.
...
**All day** means the total currently needed across all active tickets.
...
When the Barista order is complete, Barista groups all its items together, places the ticket with them on the handoff table, calls the order ready, and releases it to Service. The Barista ticket moves to the stabber only after the order leaves Barista.
...
After delivering any item, ask whether the guest needs anything else or is still waiting on Line Cooks or Barista. Leave the queue number if anything remains outstanding; retrieve it promptly when the order is complete. Queue numbers are not transferable.
...
- guests arriving or a queue building;
- orders entering Square;
- tickets building;
- Line Cooks or Barista actively producing;
- food/drinks waiting for handoff;
- guests eating;
- beverages getting low;
- finished plates;
- dirty tables needing reset;
- dishes beginning to build;
- another team becoming overwhelmed.
...
Earlier classes may reset for the next class. The final responsible class completes shutdown under current station SOPs and schedule.

## Expo / Floaters Operating Loop

One trained Floater remains responsible for Expo/pass during active production ticket flow. Other trained and authorized Floaters deploy according to need through:
...
During Friday service, students operate through:

**Arrive On Time and Work Ready → Open Your Station → Own Your Area → Communicate → Stay Aware — See the Whole Operation → Be Useful → Do the Work → Clean as You Go → Reset Your Station → Close When Required**

See `../Operations/Bistro_Rhythms.md` for the complete current teaching and operating reference.
...
The food is part of the work. The larger question is whether students can work safely, professionally, and usefully together.
